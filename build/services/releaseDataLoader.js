...
export class ReleaseDataLoader {
    ...
    async performLoad(version, cacheKey) {
        const errors = [];
        ...
        const errorDetails = errors
            ...
            .join("; ");
        throw new Error(`Failed to load version ${version} from all data sources. Errors: ${errorDetails}`);
    }
    /**
     * Load multiple releases in parallel
     */
    async loadReleases(versions) {
        const results = {};
        const promises = versions.map(async (version) => {
            try {
                ...
                console.error(`Error loading ${version}: ${error instanceof Error ? error.message : String(error)}`);
            }
        });

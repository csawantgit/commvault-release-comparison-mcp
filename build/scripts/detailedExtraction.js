...
async function detailedExtraction(version, url) {
    ...
    while ((h2Match = h2Regex.exec(articleContent)) !== null) {
        ...
        sectionCount++;
        if (sectionCount >= 6)
            break; // Limit to first 6 sections
    }
    ...
    if (totalItems < 10) {
        recommendations.push(`⚠ Very few items (${totalItems}) - consider adjusting extraction depth`);
    }
    // Specific improvements
    if (overallProblems.has("Nested lists causing merged content")) {
        recommendations.push("💡 IMPROVEMENT: Split nested lists on newlines and filter out sub-items");
    }
    if (overallProblems.has("Navigation/TOC content mixed with features")) {
        recommendations.push("💡 IMPROVEMENT: Filter content containing 'Page contents', 'TOC', 'Related topics'");
    }
    if (sections.length > 0 && sections[0].extractedItems.length > 0) {
        const avgLength = sections.reduce((sum, s) => sum +
            s.extractedItems.reduce((itemSum, item) => itemSum + item.cleaned.length, 0), 0) / totalItems;
        if (avgLength < 50) {
            recommendations.push("⚠ Average extracted text is very short - may indicate incomplete parsing");
        }
        else if (avgLength > 300) {
            recommendations.push("⚠ Average extracted text is very long - may include multiple features per item");
        }
    }
    return {
        version,
...
async function main() {
    ...
    for (const { version, url } of versions) {
        try {
            ...
            report.sections.forEach((section, idx) => {
                ...
                if (section.extractedItems.length > 0) {
                    ...
                    section.extractedItems.forEach((item, itemIdx) => {
                        console.log(`\n  Item ${itemIdx + 1}:`);
                        console.log(`    Raw HTML: ${item.raw.replace(/\n/g, " ").slice(0, 100)}...`);
            ...
            console.log(`  Total Items Extracted: ${report.sections.reduce((sum, s) => sum + s.extractedItems.length, 0)}`);

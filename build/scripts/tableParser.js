...
async function parseTableFormat(version, url) {
        ...
        while ((tableMatch = tableRegex.exec(sectionContent)) !== null) {
            ...
            while ((rowMatch = rowRegex.exec(tableHtml)) !== null) {
                ...
                if (cells.length >= 3) {
                    ...
                    const linkMatches = Array.from(linksCell.matchAll(/<a[^>]*(?:href="([^"]*)")?[^>]*>([^<]+)<\/a>/gi));
                    ...
                    if (changeText.length > 500) {
                        qualityScore -= 20;
                        qualityIssues.push(`Long change text in ${sectionHeading} (${changeText.length} chars)`);
                    }
                    if (changeText.match(/^(https?:\/\/|<)/)) {
                        qualityScore -= 30;
                        qualityIssues.push(`Possible URL/HTML in change text: ${sectionHeading}`);
                    }
            ...
            console.log(`[${version}] Table in "${sectionHeading}": ${rowCount} rows → ${featuresInSection} features`);
        }
        sectionsInfo.push({
    ...
    const avgQualityScore = allFeatures.length > 0
...
async function main() {
    ...
    console.log("=".repeat(90));
    const url = "https://documentation.commvault.com/11.44/software/changes_in_long_term_support_release_11_44.html";
    try {
        ...
        result.sections.forEach((section, idx) => {
            ...
            if (section.features_extracted > 0) {
                console.log(`     ${section.rows_count} rows → ${section.features_extracted} features`);
            }
        });
        ...
        if (result.statistics.total_features_extracted >= 15) {
            ...
            console.log(`⚠️  WARNING: Only ${result.statistics.total_features_extracted} features extracted (target: 15+)`);
        }
        console.log("=".repeat(90));
    }
    catch (error) {

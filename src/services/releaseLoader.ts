...
import { ReleaseMapper, RawReleaseItem } from "./releaseMapper.js";
...
export class ReleaseLoader {
  ...
  private config: ReleaseLoaderConfig;

  constructor(config: ReleaseLoaderConfig) {
    this.config = config;
  }

  async loadFromURL(version: string): Promise<ReleaseData | null> {
    const url = this.config.urls[version];
    const releaseDate = this.config.dates[version];

    if (!url || !releaseDate) {
      console.error(
        `[ReleaseLoader] Missing URL or date for version ${version}`
      );
      return null;
    }
    ...
    try {
      ...
      console.error(
        `[ReleaseLoader] Extracted ${items.length} items from ${version}`
      );
      ...
      console.error(
        `[ReleaseLoader] Error loading ${version}: ${errorMsg}`
      );
      return null;
    }
  }
  ...
  private parseHTML(html: string, version: string): RawReleaseItem[] {
    const items: RawReleaseItem[] = [];
    ...
    try {
      ...
      if (!content) {
        console.error(
          `[ReleaseLoader] Could not find main content for ${version}`
        );
        return items;
      }
      ...
      $content("p").each((index, el) => {
        ...
        if (
          $el.parent().is("footer, nav, header") ||
        ...
        if (
          ...
          if (
            text.match(/new|support|enhanced|improve|add|change|update/i)
          ) {
            if (!sections.has(currentSection)) {
              sections.set(currentSection, []);
            }
      ...
      if (items.length < 5) {
        console.error(
          `[ReleaseLoader] Extracted only ${items.length} items, falling back to all list items`
        );
        $content("li").each((index, el) => {
      ...
      console.error(
        `[ReleaseLoader] Parse error for ${version}: ${

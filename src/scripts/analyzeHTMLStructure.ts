...
async function analyzeHTML(
  ...
  const headings: Array<{ text: string; level: string; position: number }> =
    [];
  let match;
  ...
  const navigationElements: Array<{ text: string; type: string }> = [];
  ...
  if (navigationElements.length > 0) {
    recommendations.push(
      `❌ Navigation/TOC detected (${navigationElements.length} items) - use selectors to exclude nav elements`
    );
  }

  if (lists.filter((l) => l.type === "UL").length > 0) {
    recommendations.push(
      `✓ UL lists detected - use <ul><li> selector for feature extraction`
    );
  }

  if (lists.filter((l) => l.type === "OL").length > 0) {
    recommendations.push(
      `✓ OL lists detected - consider <ol><li> for numbered features`
    );
  }

  if (contentSections.length > 0) {
    const listBased = contentSections.filter(
      (s) => s.structureType === "list-based"
    ).length;
    const paraBased = contentSections.filter(
      (s) => s.structureType === "paragraph-based"
    ).length;

    if (listBased > paraBased) {
      recommendations.push(
        `✓ Majority of sections are list-based (${listBased}/${contentSections.length}) - focus on list extraction`
      );
    } else if (paraBased > listBased) {
      recommendations.push(
        `⚠ Many paragraph-based sections (${paraBased}/${contentSections.length}) - need paragraph parsing`
      );
    }
  }
  ...
  if (mainContent.length < html.length * 0.5) {
    recommendations.push(
      `✓ Main content container identified (reduces noise by ${Math.round((1 - mainContent.length / html.length) * 100)}%)`
    );
  } else {
    recommendations.push(
      `⚠ Could not isolate main content - full HTML being parsed`
    );
  }
  ...
  if (html.includes("data-feature") || html.includes("class=\"feature")) {
    recommendations.push(
      `✓ HTML has feature-specific classes or attributes - use them`
    );
  }
  ...
  for (const { version, url } of versions) {
    try {
      ...
      analysis.structure.contentSections.forEach((section) => {
        console.log(
          `  ${section.heading} [${section.structureType}]`
        );

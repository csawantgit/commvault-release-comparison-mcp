...
const DOCUMENTATION_URLS = {
  ...
  "11.46":
    "https://documentation.commvault.com/11.46/software/changes_in_innovation_release_11_46.html",
};
...
async function main() {
  ...
  const results: Record<string, RawPageContent> = {};
  ...
  for (const [version, result] of Object.entries(results)) {
    ...
    console.log(
      `  List items: ${result.listItemsCount}`
    );
    console.log(
      `  Raw HTML: ${result.rawHtmlLength} bytes`
    );
  }

...
interface NormalizedReleaseData {
  version: string;
  releaseDate: string;
  dataSource: string;
  categories: {
    [category: string]: NormalizedChange[];
  };
  metadata: {
    fetchedAt: string;
    pageTitle: string;
...
async function fetchPageAndParse(
  version: string,
  url: string
): Promise<NormalizedReleaseData> {
  ...
  try {
    ...
    // Count total changes
    const totalChanges = Object.values(categories).reduce(
    ...
    return {
      ...
      metadata: {
        ...
        totalChanges,
        extractionNotes:
          "Extracted from live documentation using heading-based parsing",
      },
    };
  } catch (error) {
...
function extractFeaturesFromHTML(
  html: string,
  version: string
): Record<string, NormalizedChange[]> {
  const categories: Record<string, NormalizedChange[]> = {
    Virtualization: [],
    Security: [],
    Database: [],
    Storage: [],
  ...
  let changeCounter: Record<string, number> = {};
  ...
  for (let i = 0; i < headingMatches.length; i++) {
    ...
    const features = extractFeatures(
      ...
      lastCategory,
      version,
      (changeCounter[lastCategory] = (changeCounter[lastCategory] || 0))
    );
...
function determineCategory(heading: string): string {
...
function extractFeatures(
  ...
  const features: NormalizedChange[] = [];
...
async function main() {
  ...
  const results: Record<string, NormalizedReleaseData> = {};

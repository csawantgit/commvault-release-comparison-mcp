...
interface CategorizedFeature extends ExtractedFeature {
  category: string;
...
interface CategorizedOutput {
  ...
  statistics: {
    ...
    unmapped_features: number;
    mapping_coverage: string;
  };
  unmapped: CategorizedFeature[];
}

// Section to category mapping rules
const SECTION_CATEGORY_MAP: Record<string, { category: string; confidence: number }> = {
  // Virtualization
...
async function categorizeFeatures(
  version: string
  ...
  const unmapped: CategorizedFeature[] = [];
  ...
  features.forEach((feature) => {
    ...
    if (!mapping) {
      console.log(
        `[${version}] ⚠️  No mapping for section: "${feature.section}"`
      );
      unmapped.push({
        ...feature,
        category: "Unmapped",
  ...
  // Group by category
  const categorizedMap: Record<string, CategorizedFeature[]> = {};
  const categories = [
    "Virtualization",
    "Security",
    "Database",
  ...
  for (const cat of categories) {
    categorizedMap[cat] = categorizedFeatures.filter(
      (f) => f.category === cat
    );
  }
  ...
  try {
    ...
    for (const category of categories) {
      ...
      features.slice(0, 2).forEach((feature, idx) => {
        ...
        console.log(
          `    Confidence: ${feature.mapping_confidence}% | Quality: ${feature.quality_score}/100`
        );
      });
